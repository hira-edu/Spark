...
- **不可变模板**：基础二进制文件保持不变，可以进行代码签名
- **简单可靠**：无需复杂的二进制补丁或依赖编译器的占位符
- **单文件分发**：内嵌配置的自包含可执行文件

**格式概览：**
```
[原始二进制] → [384字节加密配置载荷] → [20字节页脚]
```

页脚包含：
- Magic: `SPARKCFG` (8 字节) - 标识符
- Version: `1` (uint16) - 格式版本
- Reserved: `0` (uint16) - 保留以供将来使用
- Length: `384` (uint32) - 载荷大小
- CRC32: 校验和 (uint32) - 完整性验证

每个客户端二进制文件都会收到一个独特的 AES 加密配置和随机生成的凭据。客户端在运行时读取此拖车以发现其服务器、端口和身份验证密钥。

**详细的技术文档**，请参阅 [TRAILER_FORMAT.md](./TRAILER_FORMAT.md)，其中包括：
- 完整的二进制格式规范
- 加密和安全详细信息
- 生成和读取工作流程
- 跨平台兼容性
...
**重要提示：** `releases/built` 中的干净模板必须保持无拖车状态。配置仅在客户端生成或更新期间附加。
