...
async function startWebRTC(deviceId, iceRestart = false) {
	...
	try {
		const icePath = shareToken
			? `/api/share/ice?token=${encodeURIComponent(shareToken)}`
			: `/api/device/webrtc/config?device=${deviceId}`;
		const resp = await axios.get(getBaseURL(false, icePath));
		if (resp?.data?.data?.ice) {
			...
			if (servers.length > 0) {
				iceServers = servers;
				iceLabel = icePath.includes('/share/') ? 'guest' : 'device';
			}
		}
	} catch (e) {
...
function ScreenModal(props) {
	...
	function construct() {
		if (ctx !== null) {
			...
			const path = props.shareToken
				? `api/share/desktop?token=${props.shareToken}&secret=${ua2hex(secret)}`
				: `api/device/desktop?device=${props.device.id}&secret=${ua2hex(secret)}`;
			ws = new WebSocket(getBaseURL(true, path));
	...
	function handleJSON(ab) {
		...
		if (data?.act === 'DESKTOP_WEBRTC_ANSWER') {
			...
			if (data?.act === 'DESKTOP_WEBRTC_CANDIDATE' || data?.act === 'DESKTOP_WEBRTC_ICE') {
				if (data.code && data.code !== 0) {
					return;
				}
