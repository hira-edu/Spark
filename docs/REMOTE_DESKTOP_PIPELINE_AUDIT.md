...
| WebRTC Fast Path | Preferred “smooth” path: WebRTC video + SCTP data channel input | `client/service/desktop/webrtc.go` | ICE is fetched from server (`/api/device/webrtc/config` or `/api/share/ice`) and injected into WebRTC offers (`ice_servers`) so agents don’t need per-agent TURN env setup. Windows uses Media Foundation H.264 by default; Pion interceptors enable NACK/TWCC. Configure coturn + `turn_secret` for reliable NAT traversal. |
