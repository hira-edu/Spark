...
- **Environment overrides** – ICE/TURN lists come from `SPARK_WEBRTC_ICE`, `SPARK_WEBRTC_ICE_USERNAME`, and `SPARK_WEBRTC_ICE_CREDENTIAL` on the agent side; the console can override via `window.SPARK_WEBRTC_PEER_CONFIG` or `window.SPARK_WEBRTC_ICE` for canary tenants.
